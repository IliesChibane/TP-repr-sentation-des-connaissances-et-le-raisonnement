...
if 0,
  labels1 = labels(:);  labels2 = labels(:);
  for i=1:N,
    labels1{i} = [labels1{i} '(t)'];
    labels2{i} = [labels2{i} '(t+1)'];
  end;
  
  x2 = x1+1; y2 = y1;
  dag = [adj, inter; zeros(size(inter)), adj];
  draw_layout(dag, [labels1;labels2], [node_t(:); node_t(:)], [x1 x2]/2, [y1 y2]);
end;
