function engine = pearl_inf_engine(pnet)
% PEARL_INF_ENGINE Pearl's algorithm (aka belief propagation) for polytrees
...
if acyclic(pnet.dag, directed)==0
    errordlg('The DAG is multiply connected, use Junction tree or Anytime algorithm');
...
engine = class(engine, 'pearl_inf_engine', inf_engine(pnet));
