...
msg{n}.mu = compute_mu(pnet.CPD{pnet.equiv_class(n)}, n, ps, msg,ns);
...
for n=1:N
   bel_jointe=min(msg{n}.mu, msg{n}.lambda);
   %[bel_conditionnelle] = normalise(bel_jointe);     
   % engine.marginal{n} = bel_conditionnelle;
%pour enlever la normalization
...
msg{n}.mu = compute_mu(CPD, n, ps, msg,ns);
...
for p=ps2(:)'
  ...
  lam_msg = compute_lambda_msg(CPD, n, ps, msg, p,ns);
...
lambda = min_lambda_msgs(n, cs, msg);

%%%%%%%

function mu_msg = compute_mu_msg(n, cs, msg,  c)
mu_msg = min(msg{n}.mu , min_lambda_msgs(n, cs, msg, c));

%%%%%%%%%

function lam = min_lambda_msgs(n, cs, msg, except)
...
for i=1:length(cs)
  c = cs(i);
  if c ~= except %les min des lambda from parents sauf à celui à qui on va envoyé et il s'appelle except
    lam = min(lam, msg{n}.lambda_from_child{i});
  end
end   
